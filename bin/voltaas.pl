...
if ($mode =~ /^\-?\-l/i)
{
    ...
    my $cubin = VoltaAs::Cubin->new($cubinFile);
...
elsif ($mode =~ /^\-?\-t/i)
{
    ...
    {
        my $cubin = VoltaAs::Cubin->new($file);
        my $arch  = $cubin->arch;
    ...
    exit(VoltaAs::VoltaAs::Test($fh, $reg, $all) ? 1 : 0);
}
...
elsif ($mode =~ /^\-?\-e/i)
{
    ...
    VoltaAs::VoltaAs::Extract($in, $out, $kernel->{Params});
...
elsif ($mode =~ /^\-?\-s/i)
{
    ...
    VoltaAs::VoltaAs::Extract($in, $out, []);
...
elsif ($mode =~ /^\-?\-i/i)
{
    ...
    while ($ARGV[0] =~ /^\-?\-D(\w+)/)
    {
        ...
        my $name  = $1;
        my $value = shift;
        eval "package VoltaAs::VoltaAs::CODE; our \$$name = '$value';"
    }
    ...
    my $kernel = VoltaAs::VoltaAs::Assemble($file, $include, !$noReuse, $nowarn);
...
elsif ($mode =~ /^\-?\-p/i)
{
    ...
    print $fh VoltaAs::VoltaAs::Preprocess($file, $include, $debug);
    close $fh;
}
# get version information
elsif ($mode =~ /^\-?\-v/i)
{
    print "$VoltaAs::VoltaAs::VERSION\n";
}
else
{
    print "$mode\n";
...
Usage:
  ...
  With the --reg flag it will show register bank conflicts not hidden by reuse flags.
    ...
    voltaas.pl --test|-t [--reg|-r] [--all|-a] <cubin_file | cuobjdump_sass_file>
  ...
  Works much like cuobjdump but outputs in a format that can be re-assembled back into the cubin.
    ...
    voltaas.pl --extract|-e [--kernel|-k kernel_name] <cubin_file> [asm_file]
  ...
  Include the debug flag to print out detailed scheduler info.
    ...
    voltaas.pl --pre|-p [--debug|-d] <asm_file> [new_asm_file]
  ...
  performance without any reuse or you can use it to set the flags manually in your sass.
    ...
    voltaas.pl --insert|-i [--noreuse|-n] <asm_file> <cubin_file> [new_cubin_file]

  Display version information and exit:

    voltaas.pl --version|-v
